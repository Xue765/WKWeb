...
//MARK:- 懒加载控件
    ...
    fileprivate func addWebViewKVO() {
        self.webView.scrollView.addObserver(self, forKeyPath: "contentSize", options: [.new,.old], context: nil)
    }
...
// MARK: - UIScrollViewDelegateDelegate
extension WKWebViewController: UIScrollViewDelegate{
    
    func scrollViewDidScroll(_ scrollView: UIScrollView){
        //MARK:处理滑动
        print("height \(scrollView.contentSize.height)  offset \(scrollView.contentOffset.y)")
...
//        progressView?.isHidden = isProgressHidden 好尴尬  不生效
     ...
     override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        //MARK:监听webView的contentSize
            ...
            if (new?.height != old?.height) {
                ...
                {
                    print("contentsize \(self.webView.scrollView.contentSize.height) height \(webView.frame.size.height)" )

                    //显示隐藏
                    bottomView.isHidden = !(webView.scrollView.contentSize.height <= webView.frame.size.height)
                }
    ...
    fileprivate func addBottomView() {
        bottomView = UIView(frame: CGRect(x: 0, y: self.view.frame.size.height-50, width: self.view.frame.size.width, height: 50))
